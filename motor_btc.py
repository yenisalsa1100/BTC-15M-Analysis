...
VERSION_MOTOR = "BTC_15M_PROFIT_ENGINE_V3_BALANCED_DISPERSION"
...
# Dispersión de fuentes por niveles.
# - <= 0.060%: normal, no penaliza.
# - 0.060% a 0.120%: dispersión media; permite operar solo con
#   señal fuerte y probabilidad alta.
# - > 0.120%: dispersión alta; bloquea la operación.
MAX_DISPERSION_NORMAL_PCT = 0.060
MAX_DISPERSION_MEDIA_PCT = 0.120
PROB_MIN_DISPERSION_MEDIA = 72.0
SCORE_MIN_DISPERSION_MEDIA = 34.0
...
def decidir(
    ...
    # Dispersión por niveles: una diferencia moderada entre fuentes
    # no mata automáticamente una señal fuerte.
    prob_max_pct = max(prob_arriba, prob_abajo) * 100.0

    if dispersion_fuentes_pct is not None:
        if dispersion_fuentes_pct > MAX_DISPERSION_MEDIA_PCT:
            return {
                "decision": "NO APOSTAR",
                "fuerza": "FUENTES MUY DISPERSAS",
                "probabilidad": prob_max_pct,
                "edge": None,
                "precio_entrada": None,
                "lado": None,
            }

        if dispersion_fuentes_pct > MAX_DISPERSION_NORMAL_PCT:
            if (
                score_abs < SCORE_MIN_DISPERSION_MEDIA
                or prob_max_pct < PROB_MIN_DISPERSION_MEDIA
            ):
                return {
                    "decision": "NO APOSTAR",
                    "fuerza": "DISPERSION MEDIA SIN FUERZA",
                    "probabilidad": prob_max_pct,
                    "edge": None,
                    "precio_entrada": None,
                    "lado": None,
...
def main():
    ...
    print(" MODO: PROFIT ENGINE V3 BALANCED DISPERSION")
