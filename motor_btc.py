...
#
# VERSION:
# BTC_15M_PROFIT_ENGINE_V3_BALANCED
#
# OBJETIVO:
# - SOLO BTC
# - KALSHI = TARGET OFICIAL
# - KALSHI = RESULTADO FINAL
# - CF BENCHMARKS / BRTI
# - COINBASE
# - KRAKEN
# - COINMARKETCAP
...
VERSION_MOTOR = "BTC_15M_PROFIT_ENGINE_V3_BALANCED"
...
# VENTANA DE ENTRADA
#
# EMPIEZA DESPUES DE 55 SEGUNDOS.
# PUEDE ENTRAR DURANTE EL CONTRATO
# MIENTRAS QUEDEN MAS DE 2 MINUTOS.
# ============================================================
...
#
# NECESITA 3 DE LAS 4.
# CF ES MUY IMPORTANTE PERO NO BLOQUEA EL MOTOR
# SI LAS OTRAS 3 ESTAN DISPONIBLES.
# ============================================================

MIN_FUENTES_OPERAR = 3

REQUIERE_CF_PARA_OPERAR = False

MAX_DISPERSION_FUENTES_PCT = 0.150
...
def manejar_senal(signum, frame):
    ...
    print("")
    print("[STOP] Señal de cancelación recibida.")
...
def mediana_valida(valores):
    ...
    return statistics.median(salida)
...
SESSION.headers.update(
    {
        "User-Agent":
        "Kalshi-BTC-15M-Profit-Engine/3.0",
...
def enviar_telegram(analisis):
    ...
    texto = (
        ...
        f"Probabilidad: "
        f"{analisis.get('probabilidad', 0):.1f}%\n"
        f"Target Kalshi: "
        f"${analisis.get('target', 0):,.2f}\n"
        f"BTC: "
        f"${analisis.get('precio_consenso', 0):,.2f}\n"
        f"Hora entrada: "
        f"{analisis.get('hora_prediccion_local', '-')}\n"
    )
...
def obtener_coinbase_candles():
    ...
    # Solo velas totalmente cerradas.
...
def orderflow_coinbase(trades):
    ...
    for trade in trades:
        ...
        try:
            ...
            # Coinbase side representa maker.
            if side == "sell":
...
def calcular_score(
    ...
    if consenso["total"] == 4:
        ...
        if consenso["arriba"] == 3:
            ...
            razones.append(
                "3/3 fuentes disponibles arriba"
            )
...
def decidir(
    ...
    # ========================================================
    # CF NO ES BLOQUEO ABSOLUTO EN V3
    # ========================================================

    if (
        REQUIERE_CF_PARA_OPERAR
        and not cf_disponible
    ):
    ...
    if (
        dispersion_fuentes_pct
        is not None
        and dispersion_fuentes_pct
        > MAX_DISPERSION_FUENTES_PCT
    ):
    ...
    if score > 0:
        ...
        # FUERTE
        #
        # PUEDE PASAR INCLUSO EN ZONA MUERTA.
        # ====================================================
...
def actualizar_resultados():
    ...
    for registro in historial:
        ...
        fecha_resultado_local = (
            iso_local()
        )

        hora_resultado_local = (
            formato_hora_local()
        )
        ...
        registro[
            "resultado_hora_local"
        ] = hora_resultado_local

        registro[
            "resultado_timestamp_local"
        ] = fecha_resultado_local
...
def analizar_mercado(mercado):
    ...
    return {
        ...
        "hora_prediccion_local":
        hora_pred_local,

        "prediccion_timestamp_local":
        fecha_pred_local,
        ...
        "volumen_promedio20":
        indicadores["volume_avg20"],

        "volumen_relativo":
        indicadores["volumen_relativo"],
...
def guardar_si_corresponde(
    ...
    if (
        ...
        print(
            "[FINAL] "
            "Contrato guardado como NO APOSTAR."
        )
...
def mostrar_estadisticas():
    ...
    for x in resueltas[-10:]:
        ...
        print(
            ...
            f"-> {x.get('resultado')} | "
            f"Predicción: "
            f"{x.get('hora_prediccion_local', '-')} | "
            f"Resultado: "
            f"{x.get('resultado_hora_local', '-')}"
        )
...
def main():
    ...
    print(
        f" VERSION: {VERSION_MOTOR}"
    )
    print(
        " MODO: PROFIT ENGINE V3 BALANCED"
    )
    print(
        " DECISIONES: "
    ...
    print(
        " SCORE MEDIO/FUERTE: "
        f"{SCORE_MEDIO}/{SCORE_FUERTE}"
    )
    print(
        " EDGE MEDIO/FUERTE: "
        f"{EDGE_MINIMO_MEDIO * 100:.1f}%/"
        f"{EDGE_MINIMO_FUERTE * 100:.1f}%"
    )
    print(
        " MAX DISPERSION: "
        f"{MAX_DISPERSION_FUENTES_PCT:.3f}%"
    )
    print(
        " CF OBLIGATORIO: "
        f"{REQUIERE_CF_PARA_OPERAR}"
    )
    print(
        " NO COLOCA ORDENES REALES"
    )
